...
#include "monocular_pose_estimator/monocular_pose_estimator_cvcamera.h"
...
MPENode::MPENode(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
  ...
  // video_capture_.set(CV_CAP_PROP_MODE, CV_CAP_PROP_CONVERT_RGB);
...
void MPENode::run() {
  ...
  if(!video_capture_.retrieve(image)) {
    ROS_WARN("Could not retrieve image!");
    return;
  }
